...
async fn read<S : AsyncReadExt + Unpin>(source: &mut S) -> Result<Packet, Error> {
    ...
    assert!(null_padding == [0, 0], "Packet did not include expected 2 null bytes");
    
    Ok(Packet { id: request_id, packet_type, payload })
}
...
mod tests {
    ...
    async fn test_read_packet() -> Result<(), Error> {
        ...
        assert_eq!(packet, Packet { id: 123, packet_type: PacketType::Command, payload: "test command".to_owned() });
        Ok(())
    }
}